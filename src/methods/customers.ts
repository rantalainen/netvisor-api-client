...
export class NetvisorCustomerMethod extends NetvisorMethod {
    ...
    if (xmlObject.customerlist.customer) {
      forceArray(xmlObject.customerlist.customer).forEach((xmlCustomer) => {
        ...
        customerList.push({
          ...
          customerGroupID: parseInt(xmlCustomer.customergroupid),
  ...
  async getCustomer(params: GetCustomerParameters): Promise<GetCustomer[]> {
    ...
    if (xmlObject.customers.customer) {
      forceArray(xmlObject.customers.customer).forEach((xmlCustomer) => {
        ...
        const customer: GetCustomer = {
          ...
          customerAdditionalInformation: {
            ...
            balanceLimit: parseFloat(xmlCustomer.customeradditionalinformation.balancelimit.replace(',', '.')),
   ...
   */
  async customer(customer: Customer, params: CustomerParameters) {
    return await this._client.post('customer.nv', buildXml({ root: { customer: customer } }), params);
  }
}
